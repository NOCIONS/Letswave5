function [outheader,outdata] = LW_chunkepochs(header,data,offset,epochlength,epochstep,numchunks);
...
%       accumulation of rounding errors. 
        dx1 = round(dx_offset+((chunkpos-1)*dx_epochstep));
...
%         dx2_store(chunkpos) = dx2;
        ...
        %x1,x2
        x1=((dx1-1)*header.xstep)+header.xstart;
        x2=((dx2-1)*header.xstep)+header.xstart;
        %search for events
                ...
                if header.events(eventpos).epoch==epochpos;
                    if header.events(eventpos).latency>=x1;
                        if header.events(eventpos).latency<=x2;
                            ...
                            outheader.events(neweventpos).latency=header.events(eventpos).latency-x1;
