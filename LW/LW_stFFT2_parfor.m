function [outheader,outdata] = LW_stFFT2_parfor(header,data,freqstart,freqstep,freqsize,timestart,timestep,timesize,windowsize,windowtype,postproc,average)
% LW_stFFT2
%
% Inputs
% - header (LW5 header)
...
for chanpos=1:header.datasize(2);
    for indexpos=1:header.datasize(3);
        for dz=1:header.datasize(4);
            ...
            %stfft
            [S,P]=sub_tfa_stft_parfor(x,xtimes,t,f,Fs,winsize,wintype,padvalue);
            S=permute(S,[3,1,2]);
            P=permute(P,[3,1,2]);
            if strcmpi(postproc,'none');
