function LW_createPlugin(varargin)
    ...
    savePlugin = 'yes';
    if isempty(which('n'))
        savePlugin = questdlg(sprintf('Attention: the function you added does not belong to the current matlab path. You will either have to add the funcion location to the path, or copy your function to the plugins folder in the letswave path.\n\nDo you want to add the plugin anyway?'), 'function not in path', 'yes', 'no', 'no');
    end
