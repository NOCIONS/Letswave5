function LW_check_splinefiles(lookpath,spline_extension)
% LW_check_matrixfiles
% check consistency of matrixfiles
%
% Inputs
% - lookpath : path to check for consistency of matrixfiles
% - spline_extension : extension of splinefiles (usually, '*.spl')
%
% Outputs : none.
%
% Dependencies : none.
...
filepath=lookpath;
%find spline files
tp=dir([lookpath,filesep,spline_extension]);
for i=1:size(tp,1);
    splinefiles{i}=tp(i).name;
end;
%splineindex
$$$
