...
for filepos=1:length(inputfiles);
    ...
    %number of epochs
    numepochs=header.datasize(1);
    disp(['Number of epochs : ' num2str(numepochs)]);
    origdata=data;
    origheader=header;
    selectedchannels=1:header.datasize(2);
    selectedindexes=1:header.datasize(3);
    for splitpos=1:numsplit;
        ...
        [header,data]=LW_selectsignals(origheader,origdata,di1:di2,selectedchannels,selectedindexes);
        %save header
        [p,n,e]=fileparts(inputfiles{filepos});
        st=[p,filesep,'split',num2str(splitpos),' ',n,'.lw5'];
        disp(['saving ',st]);
        save(st,'-MAT','header');
        %save data
        [p,n,e]=fileparts(inputfiles{filepos});
        st=[p,filesep,'split',num2str(splitpos),' ',n,'.mat'];
        save(st,'-MAT','data');
    end;
end;
disp('*** Finished.');
