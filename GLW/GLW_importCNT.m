function varargout = GLW_importCNT(varargin)
% GLW_IMPORTCNT MATLAB code for GLW_importCNT.fig
%
% Author : 
% André Mouraux
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @GLW_importCNT_OpeningFcn, ...
                   'gui_OutputFcn',  @GLW_importCNT_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GLW_importCNT is made visible.
function GLW_importCNT_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% Choose default command line output for GLW_importCNT
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);
% UIWAIT makes GLW_importCNT wait for user response (see UIRESUME)
...
function varargout = GLW_importCNT_OutputFcn(hObject, eventdata, handles) 
...
update_status.function(update_status.handles,'*** Importing CNT data.',1,0);
for filepos=1:length(filenames);
    update_status.function(update_status.handles,['Importing : ' filenames{filepos}],1,0);
    [header,data]=LW_importCNT(filenames{filepos});
