...
update_status.function(update_status.handles,'*** Merging epochs.',1,0);
...
for filepos=2:length(selectedfiles);
    ...
    %process
    update_status.function(update_status.handles,'Merging epochs',1,0);
    [header1,data1]=LW_merge_epochs(header1,data1,header2,data2);
end;
%save header data
[p2,n,e]=fileparts(get(handles.outputedit,'String'));
st=[p,filesep,n,'.lw5'];
LW_save(st,[],header1,data1);
...
set(handles.listbox2,'String',stringlist(currentselection));
%check position
if deleteselection>length(currentselection);
    set(handles.listbox2,'Value',length(currentselection));
end;
