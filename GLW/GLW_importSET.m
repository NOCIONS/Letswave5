function varargout = GLW_importSET(varargin)
% GLW_IMPORTSET MATLAB code for GLW_importSET.fig
%
% Author : 
% André Mouraux
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @GLW_importSET_OpeningFcn, ...
                   'gui_OutputFcn',  @GLW_importSET_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GLW_importSET is made visible.
function GLW_importSET_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% Choose default command line output for GLW_importSET
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);
% UIWAIT makes GLW_importSET wait for user response (see UIRESUME)
...
function varargout = GLW_importSET_OutputFcn(hObject, eventdata, handles) 
...
update_status.function(update_status.handles,'*** Importing EEGLAB SET data.',1,0);
for filepos=1:length(filenames);
    update_status.function(update_status.handles,['Importing : ' filenames{filepos}],1,0);
    [header,data]=LW_importSET(filenames{filepos});
