function varargout = GLW_importVHDR(varargin)
% GLW_IMPORTVHDR MATLAB code for GLW_importVHDR.fig
%
% Author : 
% André Mouraux
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @GLW_importVHDR_OpeningFcn, ...
                   'gui_OutputFcn',  @GLW_importVHDR_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GLW_importVHDR is made visible.
function GLW_importVHDR_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% Choose default command line output for GLW_importVHDR
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);
% UIWAIT makes GLW_importVHDR wait for user response (see UIRESUME)
...
function varargout = GLW_importVHDR_OutputFcn(hObject, eventdata, handles) 
...
update_status.function(update_status.handles,'*** Importing VHDR data.',1,0);
for filepos=1:length(filenames);
    update_status.function(update_status.handles,['Importing : ' filenames{filepos}],1,0);
    [header,data]=LW_importVHDR(filenames{filepos});
