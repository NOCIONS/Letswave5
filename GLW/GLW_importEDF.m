function varargout = GLW_importEDF(varargin)
% GLW_IMPORTEDF MATLAB code for GLW_importEDF.fig
%
% Author : 
% Andr Mouraux
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @GLW_importEDF_OpeningFcn, ...
                   'gui_OutputFcn',  @GLW_importEDF_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GLW_importEDF is made visible.
function GLW_importEDF_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% Choose default command line output for GLW_importEDF
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);
% UIWAIT makes GLW_importEDF wait for user response (see UIRESUME)
...
function varargout = GLW_importEDF_OutputFcn(hObject, eventdata, handles) 
...
update_status.function(update_status.handles,'*** Importing EDF data.',1,0);
for filepos=1:length(filenames);
    update_status.function(update_status.handles,['Importing : ' filenames{filepos}],1,0);
    [header,data]=LW_importEDF(filenames{filepos});
    %is continuous?
    if get(handles.continuous_chk,'Value')==1;
        data2=[];
        update_status.function(update_status.handles,'Imported data was defined as continuous, concatenating trials and events if needed.',1,0);
        new_datasize=header.datasize;
        new_datasize(1)=1;
        new_datasize(6)=header.datasize(1)*header.datasize(6);
        for epochpos=1:header.datasize(1);
            data2=cat(6,data2,data(epochpos,:,:,:,:,:));
        end;
        %fix events
        for eventpos=1:length(header.events);
            header.events(eventpos).latency=header.events(eventpos).latency+(header.datasize(6)*header.xstep);
            header.events(eventpos).epoch=1;
        end;
        data=data2;
    ...
    if get(handles.BIOSIG_chk,'Value')==1;
        update_status.function(update_status.handles,'BIOSIG functions will be used to import events.',1,0);
        header.events=[];
        HDR=sopen_modified(filenames{filepos},'r');
        evt=bdf2biosig_events(HDR);
        evt=evt.EVENT;
        for i=1:length(evt.TYP);
            header.events(i).latency=((evt.POS(i)-1)*header.xstep)+header.xstart;
            header.events(i).code=evt.CodeDesc{evt.TYP(i)};
