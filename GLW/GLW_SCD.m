function varargout = GLW_SCD(varargin)
% GLW_SCD MATLAB code for GLW_butterhigh.fig
%
% Author : 
% André Mouraux
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @GLW_SCD_OpeningFcn, ...
                   'gui_OutputFcn',  @GLW_SCD_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function GLW_SCD_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = GLW_SCD_OutputFcn(hObject, eventdata, handles) 
...
update_status.function(update_status.handles,'*** Scalp Current Density (SCD).',1,0);
%loop through files
for filepos=1:length(inputfiles);
    ...
    if chanok==0;
        msgbox('No electrode information!');
        return;
    end;
    %compute G and H
    update_status.function(update_status.handles,'Computing G and H. This may take a while!',1,0);
    [G,H]=LW_SCD_getGH(header);
    %apply G and H
    update_status.function(update_status.handles,'Computing the CSD transform, this may take a while!',1,0);
    [header,data]=LW_SCD_apply(header,data,G,H);
