function varargout = GLW_importBDF(varargin)
% GLW_IMPORTBDF MATLAB code for GLW_importBDF.fig
%
% Author : 
% Andr Mouraux
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @GLW_importBDF_OpeningFcn, ...
                   'gui_OutputFcn',  @GLW_importBDF_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GLW_importBDF is made visible.
function GLW_importBDF_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% Choose default command line output for GLW_importBDF
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);
% UIWAIT makes GLW_importBDF wait for user response (see UIRESUME)
...
function varargout = GLW_importBDF_OutputFcn(hObject, eventdata, handles) 
...
update_status.function(update_status.handles,'*** Importing BDF data.',1,0);
for filepos=1:length(filenames);
    update_status.function(update_status.handles,['Importing : ' filenames{filepos}],1,0);
    [header,data]=LW_importBDF(filenames{filepos});
